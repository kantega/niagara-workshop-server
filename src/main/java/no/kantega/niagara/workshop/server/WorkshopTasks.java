...
public class WorkshopTasks {
    ...
    private static F<String, String> task1desc = id ->
      "start:Hei og velkommen til workshop om eventdreven applikasjonsutvikling.\n" +
        "I denne workshoppen går oppgavene ut på å sende `ProducerRecord` meldinger (og etterhvert motta `ConsumerRecord` meldinger) til " +
        "en meldingsbroker. En event-drevet arkitektur er ikke avhengig av bestemte meldignestyper, men de aller fleste rammeverk krever en viss formening " +
        "om topics eller keys , og man vil i praksis måtte forholde seg til dette. Her har vi lagt dette inn i ProducerRecord.\n" +
        "I en event-drevet arkitektur er det stort sett en datatype man må kjenne godt til: En type som representere en (uendelig) strøm av data " +
        "I denne workshoppen kan du f.eks. bruke _Niagara_ til å modellere datastrømmer og her kalles en slik datakilde for `Source`. Den omkapsler det faktum at man ikke vet når " +
        "en verdi oppstår (den abstrahere vekk tidsaspektet). Du kan bruke rammeverket du selv vil, men det bør abstrahere vekk tid. \n" +
        "I først oppgave skal du lage en `Source` som emitter en melding som inneholder teamets navn.\n" +
        "Navnet må være:\n" +
        " * mer enn 3 tegn\n" +
        " * under 20 tegn\n" +
        " * inneholde små bokstaver\n\n" +
        "Send meldingen ved å bruke `Client` eller å sende meldingen direkte til ws://<server>:<port>/ws/ med formatet {topic:/solution/" + id + ", msg: _mld_ }";

    private static String task2desc =
      "task:Man kan endre output til en `Source` meldig for melding med `map()`. " +
        "Bruk dette til å endre datakilden din til å sende ut teamnavnet i UPPERCASE. Man kan også bruke `map()` for å først lage" +
    ...
    private static String task4desc =
      "task:En nyttig ting med datakilder er at man kan sette dem sammen slik at en starter å emitte meldinger når den forrige er ferdig. " +
        "Dette kan brukes for å sette sammen ulike kilder, men også implementere retry, failover osv... I Niagara bruker man `append()` for å " +
        "å sette sammen streams. \n" +
    ...
    private static String task5desc =
      "task:Nå som dere har fått lekt dere litt med api'et, er det på tide å se litt større på det. \n" +
        "En event-drevet applikasjon er egentlig et program som har events som input, og events som output. Ganske komplisert ikkesant?" +
        "En tommelfingerregel er at det kun produseres output basert på input, og at output er et asynkront resultat av input." +
        "\n" +
        "Neste oppgave går ut på å lage et program som bare echoer input til output. Niagara har en datatype for programmer som har en datakilde" +
        "som input, og en som output: ´Stream´. Bruk `Client.websocket(host,port,topic[])` og abonnér på topic 'echo'. Bruk så `Client.WS.run(Stream<A,B>)`" +
        "for å kjøre streamen.";

    public static String tasc6desc =
      "task:Nå skal vi implementere et materialized views som ble nevnt innledningsvis. Lag en left fold av samme source som forrige oppgave. Implementer" +
    ...
    public static String tasc7desc =
      "task:Nå kommer endelig en reell oppgave:\n" +
        "Dere skal nå lage en server som tar i mot en strøm av meldinger fra et ukjent men stort antall brukere. Meldingene har følgende format:\n" +
        "`join:[brukerid]:[gruppeId]`\n" +
        "`leave:[brukerId]:[gruppeId]`\n" +
        "`create:[gruppeId]`\n" +
        "`message:[userId]:[text]` - der `text` kan inneholde en _mention_, i dette tilfellet `@[brukerId]\n" +
        "\n" +
        "Dere skal lage en applikasjon som sender et varsel til en bruker når han blir _mentioned_ i en gruppe som _vedkommende er medlem i_ (man kan blir mentioned " +
        "i grupper man er medlem i også, de skal ignoreres).\n" +
    ...
    public static final List<String> echoStrings = List.arrayList("e", "c", "h", "oo", "oo");

    public static final List<String> foldStrings = List.arrayList("e", "ec", "ech", "echoo", "echoooo");
    ...
    private final static Seq<String> nays =
      Seq.arraySeq("Hmm...", "Ikke heeelt...", "Tjaa...", "Naai...", "Prøv en gang til...")
        .map(str -> str + "\n");
