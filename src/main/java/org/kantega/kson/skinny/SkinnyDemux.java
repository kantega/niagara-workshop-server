...
public class SkinnyDemux<A> implements JsonDecoder<A> {
    ...
    public static <A> org.kantega.kson.skinny.SkinnyDemux<A> demuxer() {
        TreeMap<String, JsonDecoder<A>> decoders =
          TreeMap.empty(Ord.stringOrd);
        return new org.kantega.kson.skinny.SkinnyDemux<>(decoders);
    }

    public org.kantega.kson.skinny.SkinnyDemux<A> add(String name, JsonDecoder<? extends A> decoder) {
        return new org.kantega.kson.skinny.SkinnyDemux<>(decoders.set(name, (JsonDecoder<A>) decoder));
    }
