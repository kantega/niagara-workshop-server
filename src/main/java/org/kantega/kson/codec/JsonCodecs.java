...
public class JsonCodecs {
    ...
    public static JsonCodec<String> stringCodec =
      JsonCodec.JsonCodec(stringEncoder, stringDecoder);

    public static JsonCodec<BigDecimal> bigDecimalCodec =
      JsonCodec.JsonCodec(bigDecimalEncoder, bigDecimalDecoder);
    ...
    public static JsonCodec<Boolean> booleanCodec =
      JsonCodec.JsonCodec(boolEncoder, boolDecoder);

    public static <A> JsonCodec<Option<A>> optionCodec(JsonCodec<A> a) {
        return JsonCodec.JsonCodec(optionEncoder(a.encoder), optionDecoder(a.decoder));
    }

    public static <A> JsonCodec<List<A>> arrayCodec(JsonCodec<A> a) {
        return JsonCodec.JsonCodec(arrayEncoder(a.encoder), arrayDecoder(a.decoder));
    }

    public static <A> JsonCodec<TreeMap<String, A>> fieldsCodec(JsonCodec<A> a) {
        return JsonCodec.JsonCodec(fieldsEncoder(a.encoder), fieldsDecoder(a.decoder));
    }
    ...
    public static <A, X> JsonCodec<X> objectCodec(
      ...
      F<A, X> to
        ...
        return JsonCodec.JsonCodec(JsonEncoders.obj(a.encoder).contramap(from), JsonDecoders.obj(a.decoder).map(to));
    }
